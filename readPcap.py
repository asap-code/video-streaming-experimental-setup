                ...
                if (p[IP_SRC]==t[IP_SRC] or p[IP_SRC]==t[IP_DST]) and\
                   ...
                   (p[DPORT]==t[SPORT] or p[DPORT]==t[DPORT]) and\
                    ...
                    if p[IP_SRC]==t[IP_SRC]:
                        ...
                        DLdata+=int(p[IP_LEN])
                        lastDLTimestamp=float(p[TS_START])#IF ACK RECORD TIME AND STOP SUMING PACKET SIZE IN THE DOWNLOAD DIRECTION 
...
def getTPInterTimePacketSize(maxTuples,packets,machineIP):
    ...
    for p in packets:
        ...
        if p[IP_SRC]==srcIP:#p[IP_DST]==maxTuples[0][IP_DST] and p[IP_SRC]==maxTuples[0][IP_SRC] :#UPLINK
            ...
            if len(prevDL)!=0:
                #print(prevDL[TS_START],p[TS_START])
                #just change it from UL to DL
                DLinterTimes.append(1000.0*(float(p[TS_START])-float(prevDL[TS_START])))
            prevDL=p
            dl=dl+int(p[IP_LEN])
            if t_start==0:
            ...
            if diff_v <= bin_v:#was commented
                ...
                diff_v=float(p[TS_START])-t_start_v
                t_start_v=float(p[TS_START])#'''was commented
...
if __name__=="__main__":
    ...
    tm=time.time()
    #pcapStats,chunkInfoPcap,cdnIPs=getPcapFeature("/Users/obelmouk/youtube.pcapng","138.96.207.160",["r2---sn-gxo5uxg-jqbe.googlevideo.com"])#

...
from readPcap import getPcapFeature
from readPcap import getGoogleTuples
from readPcap import getPackets
...
SERVER_PORT=8000
#office_interface="en4" removed for tests from home
interface="enp0s3"
chromeExtension="icmmogopnhlgndejncobacigpjokdjgo"#"fifmhpcpkaingagnbnmnlmolpimjkdmi"#"bdbejdogckpdpdajccdgcljmnihcbmkg"
args=""#no-sandbox
def get_ip_address():
...
#added this to change ip address in the getpacket function
machineIP=get_ip_address()
...
def getPointAndVideo():
    ...
    #print ("getPointAndConfigureQoSBefore")#commented by me
    ...
    if headers["STOPFLAG"]=="0":
        ...
        pcapProcess=subprocess.Popen(["sudo","tcpdump","-i","enp0s3","port","443","-s","0","-w","youtube.pcap"])
...
def get_view_port():
    view_ports = ["240x144","640x360","850x480","1280x720","400x225","240x144","640x360","850x480","1280x720","400x225","240x144","640x360","850x480","1280x720","400x225"]
    return view_ports[np.random.randint(len(view_ports))]
...
class myHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if state=="WAIT":
            ...
            if self.path=="/getVideoID_Res":
                ...
                self.send_header("view_port",view_port)
                self.end_headers()
            if self.path=="/configureQoS":
                #print ("Configuring QOS",self.path)#commanted by me
                #configNetworkQoS()
                self.send_response(200)
                self.send_header("Access-Control-Allow-Origin","*")
    ...
    def do_POST(self):#do_GET(self):
        ...
        if str(queryDict[b'ts_start_js'][0])!="-1":
            ...
            os.system("sudo kill %s" % (pcapProcess.pid, ))
            #subprocess.pop
            pcapSize=0
            packetsUDPTCP=[]
            tuples=[]
            ...
            t1=time.time()
            pcapStats,chunkInfoPcap,cdnIPs=getPcapFeature("youtube.pcap",clientIP,cdns)
            #"138.96.203.5"
            packetsUDPTCP,tuples=getPackets("youtube.pcap",cdnIPs,machineIP)
            cdnTuples=getGoogleTuples(cdns,tuples)
            cdnIPs=np.unique(np.array(cdnTuples)[:,2])
            print(np.unique(np.array(cdnTuples)[:,2]))
           # print ("pcapStatsTIme",time.time()-t1) #commented by me
            import shutil
        ...
        #print ("point:",point)#commentd by me
...
try:
    ...
    ts_start_python=int(time.time()*1000)
    #resetNetworkQoS()
    #subprocess.call(["/Applications/Google Chrome.app"])#"/Applications/\"Google Chrome.app\"/Contents/MacOS/\"Google Chrome\" --kiosk")
    #cmd = '/Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome --kiosk'
